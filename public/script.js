...
async function testAPIConnection() {
    try {
        ...
        showError('loginPasswordError', 'Cannot connect to server');
        return false;
    }
}
...
async function checkAuth() {
    ...
    console.log('No valid auth found, showing welcome screen');
    showWelcomeScreen();
}
...
function setupEventListeners() {
    ...
    document.querySelectorAll('[data-time]').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            console.log('Time control set to:', gameTime, 'seconds');
        });
    });
}
...
async function login(email, password) {
    ...
    try {
        ...
        if (!response.ok || !data.success) {
            showError('loginPasswordError', data.error || 'Login failed. Please check your credentials.');
            return;
        }
...
async function signup(username, email, password) {
    ...
    try {
        ...
        if (!response.ok || !data.success) {
            showError('signupEmailError', data.error || 'Signup failed. Please try again.');
            return;
        }
